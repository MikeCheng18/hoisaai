"""This module implements the Logistic Regression model for classification tasks."""
...
from hoisaai.layer_1.model.supervised.core.linear import (
...
class LogisticRegression(
    ...
    def fit(
        ...
        for _ in range(self.maximum_iteration):
            ...
            assert (
                bool((self.beta == Tensor.Constant.INF.value).any(axis=None)) is False
            )
            assert (
                bool((self.beta == -1 * Tensor.Constant.INF.value).any(axis=None))
                is False
            )
